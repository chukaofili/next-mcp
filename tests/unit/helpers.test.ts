...
import { buildShadcnInitCommand, filteredWorkspaceCommand, formatExecDiagnostic, getAppPath, getAuthConfigRelPath, getAuthGenerateScript, getAuthSchemaOutputRelPath, getShadcnRunner, packageRunnerDlx, rewriteImportsInTree, runScriptCommand, substituteCatalog, substituteDockerfilePlaceholders, substituteProjectName, verifyAuthSchemaGenerated } from '../../src/index.js';
...
describe('verifyAuthSchemaGenerated (B6)', () => {
  let tempDir: string;
  ...
  it('drizzle full mode: passes when packages/db/src/schema/auth.ts exists', async () => {
    const cfg = createMockConfig({
      architecture: {
        monorepo: 'full',
        database: 'postgres',
    ...
    const schemaAbs = path.join(tempDir, 'packages/db/src/schema/auth.ts');
    await fs.mkdir(path.dirname(schemaAbs), { recursive: true });
    await fs.writeFile(schemaAbs, '// generated by auth CLI\n');

    expect(verifyAuthSchemaGenerated(cfg, tempDir)).toBe(true);
  });

  it('drizzle full mode: fails with reason when schema file missing (B1 silent no-op)', () => {
    const cfg = createMockConfig({
      architecture: {
        monorepo: 'full',
        database: 'postgres',
    ...
    const result = verifyAuthSchemaGenerated(cfg, tempDir);
    expect(result).toMatchObject({ ok: false });
    if (result === true) throw new Error('expected failure');
    expect(result.reason).toContain('packages/db/src/schema/auth.ts');
  });

  it('prisma full mode: passes when schema.prisma contains model User', async () => {
    const cfg = createMockConfig({
      architecture: {
        monorepo: 'full',
        database: 'postgres',
    ...
    const schemaAbs = path.join(tempDir, 'packages/db/prisma/schema.prisma');
    await fs.mkdir(path.dirname(schemaAbs), { recursive: true });
    await fs.writeFile(
      schemaAbs,
  ...
  it('prisma full mode: fails with reason when schema.prisma has no User model', async () => {
    const cfg = createMockConfig({
      architecture: {
        monorepo: 'full',
        database: 'postgres',
    ...
    const schemaAbs = path.join(tempDir, 'packages/db/prisma/schema.prisma');
    await fs.mkdir(path.dirname(schemaAbs), { recursive: true });
    await fs.writeFile(schemaAbs, `generator client { provider = "prisma-client-js" }\n`);

    const result = verifyAuthSchemaGenerated(cfg, tempDir);
    expect(result).toMatchObject({ ok: false });
    if (result === true) throw new Error('expected failure');
    expect(result.reason).toContain('User model');
  });

  it('mongoose: short-circuits true (schema-less ORM)', () => {
    const cfg = createMockConfig({
      architecture: {
        monorepo: 'full',
        database: 'mongodb',
...
describe('formatExecDiagnostic (B6)', () => {
  it('returns empty string when result is success', () => {
    expect(formatExecDiagnostic({ success: true })).toBe('');
  });

  it('returns empty string when failure has no reason or stderr', () => {
    expect(formatExecDiagnostic({ success: false })).toBe('');
  });

  it('formats reason and stderr into a multi-line block when present', () => {
    const out = formatExecDiagnostic({
      success: false,
      reason: 'verify rejected',
      stderr: 'Error: dotenv: command not found\nat ...',
    });
    expect(out).toContain('Reason: verify rejected');
    expect(out).toContain('Captured stderr:');
    expect(out).toContain('  Error: dotenv: command not found');
  });
});
